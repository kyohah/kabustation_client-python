...
class RequestSendOrderDerivFuture(BaseModel):
    """
    RequestSendOrderDerivFuture
    """ # noqa: E501
    password: StrictStr = Field(description="注文パスワード", alias="Password")
    symbol: StrictStr = Field(description="銘柄コード<br>※取引最終日に「先物銘柄コード取得」でDerivMonthに0（直近限月）を指定した場合、日中・夜間の時間帯に関わらず、取引最終日を迎える限月の銘柄コードを返します。取引最終日を迎える銘柄の取引は日中取引をもって終了となりますので、ご注意ください。", alias="Symbol")
    exchange: StrictInt = Field(description="市場コード <table> <thead> <tr> <th>定義値</th> <th>説明</th> </tr> </thead> <tbody> <tr> <td>2</td> <td>日通し</td> </tr> <tr> <td>23</td> <td>日中</td> </tr> <tr> <td>24</td> <td>夜間</td> </tr> <tr> <td>32</td> <td>SOR日通し</td> </tr> <tr> <td>33</td> <td>SOR日中</td> </tr> <tr> <td>34</td> <td>SOR夜間</td> </tr> </tbody> </table> SOR日中、SOR夜間、SOR日通しは一部銘柄のみ対象となります。<br> SOR対象銘柄は以下をご参照ください。 <table> <thead> <tr> <th>先物SOR取扱銘柄</th> <th>有効限月</th> </tr> </thead> <tbody> <tr> <td>日経225先物ラージ</td> <td>直近2限月</td> </tr> <tr> <td>日経225先物ミニ</td> <td>直近4限月</td> </tr> <tr> <td>TOPIX先物ラージ</td> <td>直近2限月</td> </tr> <tr> <td>TOPIX先物ミニ </td> <td>直近3限月</td> </tr> <tr> <td>東証マザーズ指数先物</td> <td>直近2限月</td> </tr> <tr> <td>JPX日経400先物</td> <td>直近2限月</td> </tr> <tr> <td>NYダウ先物</td> <td>直近2限月</td> </tr> </tbody> </table>", alias="Exchange")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RequestSendOrderDerivFuture from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RequestSendOrderDerivFuture from a dict"""
        if obj is None:
            return None
